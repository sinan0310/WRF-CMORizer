...
#source loadenv.JURECA-DC_2023_Intel-PSMPI.ini # previous which has some files that cause errors during aggregation
source loadenv.JURECA-DC_2023_Intel-PSMPI_forCmorize.ini
...
# special accelerated variant, months are concatenated, dirty hack
#find . -name "*${Yn}0101*" -delete
###declare -a var_arr=("cape" "cin")
###for var in "${var_arr[@]}"; do
###	cd /p/scratch/cjjsc39/goergen1/sim/tmp_FPSCONV/tmp_DA/postpro/CMORized/CORDEX-FPSCONV/output/ALP-3/FZJ-IDL/SMHI-EC-EARTH/historical/r12i1p1/FZJ-IDL-WRF381DA/fpsconv-x1n2-v1/1hr/${var}
###	ncrcat -h ${var}_ALP-3_SMHI-EC-EARTH_historical_r12i1p1_FZJ-IDL-WRF381DA_fpsconv-x1n2-v1_1hr_${Y}*.nc tmp.nc
###	[ $? -ne 0 ] && exit 1
###	rm ${var}_ALP-3_SMHI-EC-EARTH_historical_r12i1p1_FZJ-IDL-WRF381DA_fpsconv-x1n2-v1_1hr_${Y}*.nc
###	mv tmp.nc ${var}_ALP-3_SMHI-EC-EARTH_historical_r12i1p1_FZJ-IDL-WRF381DA_fpsconv-x1n2-v1_1hr_${Y}010100-${Y}123123.nc
###done
