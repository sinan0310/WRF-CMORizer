...
The pCMORizer.f90 is an open-source Fortran-based software tool with some ancilliary bash scripts that also use [cdo](https://code.mpimet.mpg.de/projects/cdo/) to transfer or postprocess raw (regional climate) model ouputs into netCDF files, which comply to the [CMOR](https://cmor.llnl.gov) standard according to the [CORDEX-CMIP3 archive specification](http://is-enes-data.github.io/cordex_archive_specifications.pdf) as part of the [CORDEX WCRP project](https://cordex.org/experiment-guidelines/how-to-submit-data-rcms/), and make the model data compliant to be diseminated through [ESGF](https://esgf-data.dkrz.de/projects/esgf-dkrz/), or ready for efficient local use.

## Motivation, features, design principles

At an early stage of the CORDEX-CMIP3 experiment in 2013, there were very few tools available which would ingest raw RCM output and generate CMORized simulation results for dissemination and/or efficient in-house usage or archival, meeting FAIR principles. Existing solutions at the time often consisted of lengthy script-based processing chains, that resulted in lots of lengthy I/O operations, some issues when it came to the generation of more complex variables and diagnostics, or large memory footprints. In this context the pCMORizer.f90 was orginally developed as a purely serial tool for WRF RCM outputs. The orignal version was expanded by the community now and then, partly adopted to specific workflows, and further customized. Full operational capability, was only reached step by step, triggered by archival, dissemination, publication needs. Today a range of CMORizer tools exist for various RCMs, see the [Links section](#ref_links). The pCMORizer.f90 has undergone three major development stages (i.e., variants), only the most current is maintained and documented here. 

## Maintained variant

- Two-staged processing: Fortran tool does initial CMORization step, cdo does temporal aggregation
- MPI-parallel (thanks to Heimo TRUHETZ): each MPI task processes one variable independently
- Model input files are handled via sbatch run control shell script
...
- 39 near-surface variables: about 5h25min
