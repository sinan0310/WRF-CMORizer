...
## special serial variant -> this is too slow
#let nvar=2
#cp -f ../../runctrl.current.nml_template_${DOM}_${identifier} runctrl.current.nml_${DOM}
#sed -i "s/__YYYY__/$Y/g" runctrl.current.nml_${DOM}
#sed -i "s/__nvar__/$nvar/g" runctrl.current.nml_${DOM}
...
# special, accelerated variant for variables which require a lot of processing, 
# months are processed in parallel
let nvar=2
...
# a simple "wait" here is not enough, it refers only to the special variable 
# processing, this leads to a race condition, once the cape / cin is ready the 
# script stops, this happens depending on the filesystem access; usually cape / 
# cin is always the slowest, but not always
echo "processes (15): ${pid[@]}"
for jj in ${!pid[@]} ; do
  wait ${pid[${jj}]}
  if [[ $? -ne 0 ]]; then
